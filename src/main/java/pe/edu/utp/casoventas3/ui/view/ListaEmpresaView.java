...
public class ListaEmpresaView extends javax.swing.JDialog implements MVPView {
    ...
    private void CargaDatos(Object[] params){
        ...
        List<Empresa> lista = (List<Empresa>) params[0];
        lista.stream().map((item) -> {
            Object[] objs = new Object[2];
            objs[0] = item.getRucEmpresa();
            objs[1] = item.getRazSocEmpresa();
            return objs;
        }).forEachOrdered((objs) -> {
    ...
    public ListaEmpresaView(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
